...
function Search(finishMoveCallback, maxPly, finishPlyCallback) {
  ...
  g_globalPly++;
    ...
    console.log(`AlphaBeta returned for depth ${i}:`, tmp);
}
...
// Position variables
var g_board = new Array(256).fill(0x80); // Sentinel 0x80, pieces are in low 4 bits, 0x8 for color, 0x7 bits for piece type
...
function ResetGame() {
    ...
    console.log('pieceSquareAdj after initialization:', pieceSquareAdj);
...
function InitializePieceList() {
    ...
    console.log('Accessing g_board at index:', i);
...
function MakeMove(move) {
    ...
    // if (!pieceSquareAdj) {
    //     console.error("pieceSquareAdj is undefined");

    //     return false;
    // }
    // if (!pieceSquareAdj[captured & 0x7]) {
    //     console.error("pieceSquareAdj[captured & 0x7] is undefined");
    //     return false;
    // }
    ...
    if (captured) {
        ...
        g_move50 = 0;
        // console.log('g_pieceCount:', g_pieceCount[capturedType]);
        // console.log('g_pieceList:', g_pieceList);
    ...
    // console.log('pieceSquareAdj:', pieceSquareAdj);
    // console.log('Index used:', captured & 0x7);
    // console.log('Attempting Move:', move);
    // console.log('From:', from, 'To:', to, 'Piece:', piece);


    // console.log('captured:', captured);
    // console.log('epcEnd:', epcEnd);
    // console.log('me:', me);
    // console.log('pieceSquareAdj:', pieceSquareAdj[captured & 0x7]);
    // console.log('flipTable:', flipTable[epcEnd]);
...
self.onmessage = function (e) {
    ...
    if (typeof e.data === 'string' && e.data.match("^position") == "position") {
        ResetGame();
        var result = InitializeFromFen(e.data.substr(9, e.data.length - 9));
        if (result.length != 0) {
            postMessage("message " + result);
        }
    } else if (typeof e.data === 'string' && e.data.match("^search") == "search") {
...
export { InitializeFromFen, Search, FormatSquare, PVFromHash, ResetGame, FormatMove, 
    GetMoveFromString, MakeMove, GenerateValidMoves, GenerateMove, GenerateAllMoves, GenerateCaptureMoves, 
    See, FinishPlyCallback, FinishMoveLocalTesting };